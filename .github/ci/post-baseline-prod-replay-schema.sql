-- CI production post-baseline replay schema — auto-generated from production pg_dump.
-- DO NOT EDIT BY HAND. Regenerate with: bash .github/scripts/dump-replay-schema.sh --env production
...
CREATE FUNCTION public.prevent_tournament_hard_delete() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.update_br_round_results_updated_at() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    NEW.updated_at = now();
...
CREATE TABLE public.leaderboard_refresh_state (
    ...
    CONSTRAINT leaderboard_refresh_state_id_check CHECK ((id = 1))
);



CREATE TABLE public.leaderboard_team_stats (
    ...
    rp bigint DEFAULT 0 NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_leaderboard_team_stats_non_negative CHECK (((matches_played >= 0) AND (wins >= 0) AND (losses >= 0) AND ((wins + losses) <= matches_played) AND (tournaments_played >= 0) AND (tournaments_won >= 0)))
);
...
CREATE TABLE public.match_map_vetos (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    team1_picked_maps jsonb DEFAULT '[]'::json,
    team2_picked_maps jsonb DEFAULT '[]'::json,
...
CREATE TABLE public.sponsor_asset_cleanup_jobs (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE ONLY public.sponsor_asset_cleanup_jobs FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.sponsor_device_daily_stats (
    ...
    impressions bigint DEFAULT 0 NOT NULL,
    clicks bigint DEFAULT 0 NOT NULL,
    CONSTRAINT sponsor_device_daily_stats_device_class_check CHECK ((device_class = ANY (ARRAY['mobile-web'::text, 'desktop-web'::text, 'tablet-web'::text, 'unknown-web'::text, 'iphone'::text, 'ipad'::text, 'android-phone'::text, 'android-tablet'::text, 'windows-pc'::text, 'mac'::text, 'linux-pc'::text])))
);
...
CREATE TABLE public.sponsor_placement_assets (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    bucket text NOT NULL,
    object_path text NOT NULL,
    ...
    expires_at timestamp with time zone DEFAULT (now() + '24:00:00'::interval) NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT sponsor_placement_assets_asset_role_check CHECK ((asset_role = ANY (ARRAY['banner'::text, 'logo'::text]))),
    CONSTRAINT sponsor_placement_assets_bucket_check CHECK ((bucket = 'system.assets.partners'::text)),
    CONSTRAINT sponsor_placement_assets_byte_size_check CHECK ((byte_size > 0)),
    CONSTRAINT sponsor_placement_assets_height_check CHECK ((height > 0)),
    CONSTRAINT sponsor_placement_assets_mime_type_check CHECK ((mime_type = ANY (ARRAY['image/jpeg'::text, 'image/png'::text, 'image/webp'::text]))),
    CONSTRAINT sponsor_placement_assets_path_length_check CHECK (((length(object_path) <= 512) AND (length(public_url) <= 2048))),
    CONSTRAINT sponsor_placement_assets_placement_zone_check CHECK ((placement_zone = ANY (ARRAY['homepage_ticker'::text, 'partner_showcase'::text, 'sidebar_partner'::text, 'wide_partner'::text, 'card_badge'::text, 'partner_logo'::text]))),
    CONSTRAINT sponsor_placement_assets_width_check CHECK ((width > 0))
);

ALTER TABLE ONLY public.sponsor_placement_assets FORCE ROW LEVEL SECURITY;



CREATE TABLE public.sponsor_placement_audit_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.sponsor_placements (
    ...
    logo_asset_id uuid,
    description text,
    CONSTRAINT sponsor_placements_copy_length_check CHECK (((length(COALESCE(headline, ''::text)) <= 120) AND (length(COALESCE(cta_text, ''::text)) <= 60) AND (length(COALESCE(cta_url, ''::text)) <= 2048) AND (length(COALESCE(banner_url, ''::text)) <= 2048) AND (length(COALESCE(logo_url, ''::text)) <= 2048))),
    ...
    CONSTRAINT sponsor_placements_priority_check CHECK (((priority >= '-1000'::integer) AND (priority <= 1000))),
    CONSTRAINT sponsor_placements_review_reason_check CHECK (((review_reason IS NULL) OR (review_reason = ANY (ARRAY['overflow'::text, 'invalid_scope'::text, 'unassigned'::text])))),
    CONSTRAINT sponsor_placements_schedule_check CHECK (((starts_at IS NULL) OR (ends_at IS NULL) OR (starts_at < ends_at))),
    CONSTRAINT sponsor_placements_slot_check CHECK (((slot_number IS NULL) OR ((slot_number >= 1) AND (slot_number <=
CASE placement_zone
    WHEN 'homepage_ticker'::text THEN 10
    WHEN 'partner_showcase'::text THEN 6
    WHEN 'sidebar_partner'::text THEN 2
    WHEN 'wide_partner'::text THEN 4
    WHEN 'card_badge'::text THEN 1
    WHEN 'partner_logo'::text THEN 4
    ELSE NULL::integer
END))))
);
...
CREATE TABLE public.sponsor_slot_daily_stats (
    sponsor_id uuid NOT NULL,
    stat_date date NOT NULL,
    tournament_id uuid NOT NULL,
    placement_zone text NOT NULL,
...
ALTER TABLE ONLY public.sponsor_slot_daily_stats FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.tournament_cash_payouts (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    tournament_id uuid NOT NULL,
    team_id uuid NOT NULL,
    placement integer NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT tournament_cash_payouts_payment_method_check CHECK ((payment_method = ANY (ARRAY['gateway'::text, 'manual'::text])))
);

ALTER TABLE ONLY public.tournament_cash_payouts FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.tournament_disputes (
    ...
    CONSTRAINT chk_dispute_status CHECK ((status = ANY (ARRAY['open'::text, 'resolved'::text, 'rejected'::text])))
);
...
CREATE TABLE public.tournament_invitations (
    ...
    CONSTRAINT chk_tournament_invitations_email_normalized CHECK ((email = lower(TRIM(BOTH FROM email)))),
    ...
    CONSTRAINT chk_tournament_invitations_status CHECK ((status = ANY (ARRAY['draft'::text, 'sent'::text, 'redeemed'::text, 'expired'::text, 'revoked'::text])))
);
...
ALTER TABLE ONLY public.tournament_placements FORCE ROW LEVEL SECURITY;



CREATE TABLE public.tournament_reward_distributions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    tournament_id uuid NOT NULL,
    team_id uuid NOT NULL,
    placement integer NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT tournament_reward_distributions_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'distributed'::text, 'claimed'::text, 'cancelled'::text])))
);

ALTER TABLE ONLY public.tournament_reward_distributions FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.tournament_stages (
    ...
    CONSTRAINT chk_stage_format CHECK ((format = ANY (ARRAY['single_elimination'::text, 'double_elimination'::text, 'round_robin'::text, 'swiss'::text, 'battle_royale'::text]))),
    CONSTRAINT chk_stage_status CHECK ((status = ANY (ARRAY['pending'::text, 'upcoming'::text, 'active'::text, 'completed'::text]))),
...
CREATE TABLE public.tournaments (
    ...
    game_mode text,
    checkin_job_id text,
    payout_method text DEFAULT 'manual'::text NOT NULL,
    manual_payout_notes text,
    ...
    CONSTRAINT tournaments_payout_method_check CHECK ((payout_method = ANY (ARRAY['gateway'::text, 'manual'::text])))
);
...
ALTER TABLE ONLY public.leaderboard_refresh_state
    ADD CONSTRAINT leaderboard_refresh_state_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.leaderboard_team_stats
    ADD CONSTRAINT pk_leaderboard_team_stats PRIMARY KEY (team_id, game_key, region_key);
...
ALTER TABLE ONLY public.sponsor_asset_cleanup_jobs
    ADD CONSTRAINT sponsor_asset_cleanup_jobs_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.sponsor_placement_assets
    ADD CONSTRAINT sponsor_placement_assets_object_path_key UNIQUE (object_path);



ALTER TABLE ONLY public.sponsor_placement_assets
    ADD CONSTRAINT sponsor_placement_assets_pkey PRIMARY KEY (id);



ALTER TABLE ONLY public.sponsor_placement_audit_log
    ADD CONSTRAINT sponsor_placement_audit_log_pkey PRIMARY KEY (id);
...
ALTER TABLE public.sponsor_placements
    ADD CONSTRAINT sponsor_placements_scope_check CHECK (((review_reason IS NOT NULL) OR ((placement_zone = ANY (ARRAY['homepage_ticker'::text, 'partner_showcase'::text])) AND (tournament_id IS NULL)) OR ((placement_zone = ANY (ARRAY['sidebar_partner'::text, 'wide_partner'::text, 'card_badge'::text, 'partner_logo'::text])) AND (tournament_id IS NOT NULL)))) NOT VALID;



ALTER TABLE ONLY public.sponsor_slot_daily_stats
    ADD CONSTRAINT sponsor_slot_daily_stats_pkey PRIMARY KEY (sponsor_id, stat_date, tournament_id, placement_zone);
...
ALTER TABLE ONLY public.tournament_cash_payouts
    ADD CONSTRAINT tournament_cash_payouts_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.tournament_reward_distributions
    ADD CONSTRAINT tournament_reward_distributions_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.tournament_cash_payouts
    ADD CONSTRAINT uq_tournament_cash_payout_team UNIQUE (tournament_id, team_id);



ALTER TABLE ONLY public.tournament_placements
    ADD CONSTRAINT uq_tournament_placements_team UNIQUE (tournament_id, team_id);



ALTER TABLE ONLY public.tournament_reward_distributions
    ADD CONSTRAINT uq_tournament_reward_dist UNIQUE (tournament_id, team_id, reward_index);
...
ALTER TABLE ONLY public.tournament_invitations
    ADD CONSTRAINT ux_tournament_invitations_code UNIQUE (code);
...
CREATE INDEX idx_audit_logs_created_sev_type ON public.audit_logs USING btree (created_at DESC, severity, target_type);
...
CREATE INDEX idx_brkt_versions_stage_id ON public.brkt_versions USING btree (stage_id);
...
CREATE INDEX idx_leaderboard_team_stats_game_region_rp ON public.leaderboard_team_stats USING btree (game_key, region_key, rp DESC);



CREATE INDEX idx_leaderboard_team_stats_game_rp ON public.leaderboard_team_stats USING btree (game_key, rp DESC);
...
CREATE INDEX idx_match_completed_events_match ON public.match_completed_events USING btree (match_id);



CREATE INDEX idx_match_completed_events_status ON public.match_completed_events USING btree (status) WHERE (status = 'pending'::text);
...
CREATE INDEX idx_staff_audit_created_sev_type ON public.staff_audit_log USING btree (created_at DESC, severity, target_type);
...
CREATE INDEX idx_staff_audit_log_created_at ON public.staff_audit_log USING btree (created_at DESC);
...
CREATE INDEX idx_stage_participants_stage_id ON public.stage_participants USING btree (stage_id);



CREATE INDEX idx_stage_participants_team_id ON public.stage_participants USING btree (team_id);
...
CREATE INDEX idx_tournament_cash_payouts_tournament ON public.tournament_cash_payouts USING btree (tournament_id, status);
...
CREATE INDEX idx_tournament_placements_tournament ON public.tournament_placements USING btree (tournament_id, placement);



CREATE INDEX idx_tournament_reward_dist_tournament ON public.tournament_reward_distributions USING btree (tournament_id, placement);
...
CREATE INDEX sponsor_asset_cleanup_lease_idx ON public.sponsor_asset_cleanup_jobs USING btree (next_attempt_at, lease_expires_at) WHERE ((completed_at IS NULL) AND (failed_at IS NULL));



CREATE INDEX sponsor_asset_cleanup_pending_idx ON public.sponsor_asset_cleanup_jobs USING btree (next_attempt_at) WHERE (completed_at IS NULL);



CREATE UNIQUE INDEX sponsor_asset_cleanup_pending_unique_idx ON public.sponsor_asset_cleanup_jobs USING btree (bucket, object_path) WHERE ((completed_at IS NULL) AND (failed_at IS NULL));
...
CREATE INDEX sponsor_placement_assets_expiry_idx ON public.sponsor_placement_assets USING btree (expires_at) WHERE (claimed_at IS NULL);



CREATE INDEX sponsor_placement_audit_log_placement_idx ON public.sponsor_placement_audit_log USING btree (placement_id, created_at DESC);



CREATE INDEX sponsor_placement_audit_log_sponsor_idx ON public.sponsor_placement_audit_log USING btree (sponsor_id, created_at DESC);
...
CREATE INDEX sponsor_placements_inventory_idx ON public.sponsor_placements USING btree (tournament_id, placement_zone, slot_number, is_active);



CREATE INDEX sponsor_placements_review_idx ON public.sponsor_placements USING btree (review_reason, updated_at DESC) WHERE (review_reason IS NOT NULL);



CREATE UNIQUE INDEX sponsor_placements_scope_zone_slot_unique_idx ON public.sponsor_placements USING btree (COALESCE(tournament_id, '00000000-0000-0000-0000-000000000000'::uuid), placement_zone, slot_number) WHERE (slot_number IS NOT NULL);
...
CREATE TRIGGER trg_br_round_results_updated_at BEFORE UPDATE ON public.br_lobby_results FOR EACH ROW EXECUTE FUNCTION public.update_br_round_results_updated_at();
...
CREATE TRIGGER trg_prevent_tournament_hard_delete BEFORE DELETE ON public.tournaments FOR EACH ROW EXECUTE FUNCTION public.prevent_tournament_hard_delete();
...
ALTER TABLE ONLY public.leaderboard_team_stats
    ADD CONSTRAINT leaderboard_team_stats_team_id_fkey FOREIGN KEY (team_id) REFERENCES public.teams(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.match_map_vetos
    ADD CONSTRAINT match_map_vetos_stage_id_fkey FOREIGN KEY (stage_id) REFERENCES public.tournament_stages(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.sponsor_asset_cleanup_jobs
    ADD CONSTRAINT sponsor_asset_cleanup_jobs_asset_id_fkey FOREIGN KEY (asset_id) REFERENCES public.sponsor_placement_assets(id);
...
ALTER TABLE ONLY public.sponsor_placement_assets
    ADD CONSTRAINT sponsor_placement_assets_uploaded_by_fkey FOREIGN KEY (uploaded_by) REFERENCES public.profiles(id);
...
ALTER TABLE ONLY public.sponsor_placements
    ADD CONSTRAINT sponsor_placements_banner_asset_id_fkey FOREIGN KEY (banner_asset_id) REFERENCES public.sponsor_placement_assets(id);



ALTER TABLE ONLY public.sponsor_placements
    ADD CONSTRAINT sponsor_placements_logo_asset_id_fkey FOREIGN KEY (logo_asset_id) REFERENCES public.sponsor_placement_assets(id);
...
ALTER TABLE ONLY public.sponsor_slot_daily_stats
    ADD CONSTRAINT sponsor_slot_daily_stats_sponsor_id_fkey FOREIGN KEY (sponsor_id) REFERENCES public.sponsors(id) ON DELETE CASCADE;



ALTER TABLE ONLY public.sponsor_slot_daily_stats
    ADD CONSTRAINT sponsor_slot_daily_stats_tournament_id_fkey FOREIGN KEY (tournament_id) REFERENCES public.tournaments(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.stage_participants
    ADD CONSTRAINT stage_participants_stage_id_fkey FOREIGN KEY (stage_id) REFERENCES public.tournament_stages(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.tournament_cash_payouts
    ADD CONSTRAINT tournament_cash_payouts_team_id_fkey FOREIGN KEY (team_id) REFERENCES public.teams(id) ON DELETE RESTRICT;



ALTER TABLE ONLY public.tournament_cash_payouts
    ADD CONSTRAINT tournament_cash_payouts_tournament_id_fkey FOREIGN KEY (tournament_id) REFERENCES public.tournaments(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.tournament_placements
    ADD CONSTRAINT tournament_placements_team_id_fkey FOREIGN KEY (team_id) REFERENCES public.teams(id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.tournament_reward_distributions
    ADD CONSTRAINT tournament_reward_distributions_team_id_fkey FOREIGN KEY (team_id) REFERENCES public.teams(id) ON DELETE RESTRICT;



ALTER TABLE ONLY public.tournament_reward_distributions
    ADD CONSTRAINT tournament_reward_distributions_tournament_id_fkey FOREIGN KEY (tournament_id) REFERENCES public.tournaments(id) ON DELETE CASCADE;
...
CREATE POLICY sponsor_asset_cleanup_service_role_all ON public.sponsor_asset_cleanup_jobs TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY sponsor_placement_assets_service_role_all ON public.sponsor_placement_assets TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY sponsor_placement_audit_service_role ON public.sponsor_placement_audit_log TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY tournament_cash_payouts_organizer_read ON public.tournament_cash_payouts FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.tournaments t
  WHERE ((t.id = tournament_cash_payouts.tournament_id) AND ((t.organizer_id = auth.uid()) OR (t.is_public = true))))));



CREATE POLICY tournament_cash_payouts_service_role_all ON public.tournament_cash_payouts TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY tournament_placements_public_read ON public.tournament_placements FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.tournaments t
  WHERE ((t.id = tournament_placements.tournament_id) AND ((t.is_public = true) OR (t.organizer_id = auth.uid()))))));



CREATE POLICY tournament_placements_service_role_all ON public.tournament_placements TO service_role USING (true) WITH CHECK (true);



CREATE POLICY tournament_reward_dist_public_read ON public.tournament_reward_distributions FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.tournaments t
  WHERE ((t.id = tournament_reward_distributions.tournament_id) AND ((t.is_public = true) OR (t.organizer_id = auth.uid()))))));



CREATE POLICY tournament_reward_dist_service_role_all ON public.tournament_reward_distributions TO service_role USING (true) WITH CHECK (true);
