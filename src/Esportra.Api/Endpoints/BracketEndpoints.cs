...
public static class BracketEndpoints
{
    ...
    public static void MapBracketEndpoints(this WebApplication app)
    {
            ...
            CancellationToken    ct) =>
        {
            ...
            var nodes = await conn.QueryAsync<dynamic>(
                ...
                SELECT m.*,
                       l.x, l.y,
                       COALESCE(t1.name, tp1.team_name, p1.username) AS team1_name,
                       COALESCE(t1.logo_url, p1.avatar_url) AS team1_logo,
                       COALESCE(t2.name, tp2.team_name, p2.username) AS team2_name,
                       COALESCE(t2.logo_url, p2.avatar_url) AS team2_logo
                FROM brkt_matches m
                ...
                LEFT JOIN profiles p1 ON p1.id = tp1.user_id
                LEFT JOIN profiles p2 ON p2.id = tp2.user_id
        ...
        Guid versionId, IDbConnectionFactory db, CancellationToken ct)
    {
        ...
        var matches = (await conn.QueryAsync("""
            ...
            LEFT JOIN public.profiles p1 ON p1.id = tp1.user_id
            LEFT JOIN public.profiles p2 ON p2.id = tp2.user_id
            WHERE m.version_id = @versionId
            ORDER BY m.round_index, m.match_number
            """, new { versionId })).AsList();
