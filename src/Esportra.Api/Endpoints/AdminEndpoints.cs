...
public static class AdminEndpoints
{
    ...
    public static void MapAdminEndpoints(this WebApplication app)
    {
        ...
        // Returns current 2FA enforcement settings and high-level admin stats.
        app.MapGet("/api/admin/2fa/status", async (
            IDbConnectionFactory db,
            ISupabaseAdminClient supabase,
            HttpContext           ctx,
            CancellationToken    ct) =>
        {
            ...
            var settings = (await conn.QueryAsync<(string Key, string Value)>(
                new CommandDefinition(
                    ...
                    SELECT key, value
                    FROM   system_settings
                    WHERE  key IN ('security.2fa_enforcement_enabled', 'security.2fa_required_roles')
                    """,
                    cancellationToken: ct))).ToDictionary(r => r.Key, r => r.Value);

            var enforcementEnabled = settings.TryGetValue("security.2fa_enforcement_enabled", out var ev)
                && ev.Equals("true", StringComparison.OrdinalIgnoreCase);

            string[] requiredRoles;
            try
            {
                requiredRoles = settings.TryGetValue("security.2fa_required_roles", out var rv)
                    ? JsonSerializer.Deserialize<string[]>(rv) ?? Array.Empty<string>()
                    : Array.Empty<string>();
            }
            catch { requiredRoles = Array.Empty<string>(); }

            var totalAdmins = await conn.ExecuteScalarAsync<int>(
                new CommandDefinition(
                    "SELECT COUNT(DISTINCT user_id) FROM admin_user_roles",
                    cancellationToken: ct));

            // Calculate real mfaEnabledCount — cap at 50 admins for performance
            var adminUserIds = (await conn.QueryAsync<Guid>(
                new CommandDefinition(
                    "SELECT DISTINCT user_id FROM admin_user_roles LIMIT 50",
                    cancellationToken: ct))).ToList();

            int mfaEnabledCount = 0;
            foreach (var uid in adminUserIds)
            {
                try
                {
                    var factorsEl = await supabase.GetUserMfaFactorsAsync(uid, ct);
                    bool hasVerified = false;
                    if (factorsEl.ValueKind == JsonValueKind.Array)
                    {
                        hasVerified = factorsEl.EnumerateArray()
                            .Any(f => f.TryGetProperty("status", out var s) && s.GetString() == "verified");
                    }
                    else if (factorsEl.ValueKind == JsonValueKind.Object)
                    {
                        hasVerified = factorsEl.EnumerateObject()
                            .Where(p => p.Value.ValueKind == JsonValueKind.Array)
                            .SelectMany(p => p.Value.EnumerateArray())
                            .Any(f => f.TryGetProperty("status", out var s) && s.GetString() == "verified");
                    }
                    if (hasVerified) mfaEnabledCount++;
                }
                catch
                {
                    // Skip users with no factors or transient Supabase errors
                }
            }
            ...
            return Results.Ok(new
            {
                ...
                totalAdmins,
                mfaEnabledCount,
                note = "Per-user MFA status available via /api/admin/2fa/users",
            });
        }).RequireAuthorization("Admin");

        // ── GET /api/admin/2fa/users ───────────────────────────────────────────
        // Returns paginated admin users with their live MFA factor status.
        // NOTE: Calls Supabase once per user (N+1). Acceptable because admin
        //       user counts are typically < 100 and perPage is capped at 25.
        app.MapGet("/api/admin/2fa/users", async (
            IDbConnectionFactory    db,
            ISupabaseAdminClient    supabase,
            HttpContext              ctx,
            CancellationToken       ct,
            ...
            string                  mfaStatus = "all") =>
        {
            ...
            perPage = Math.Clamp(perPage, 1, 25); // Cap at 25 to limit N+1 Supabase calls

            using var conn = db.CreateConnection();

            // Load 2FA required-roles setting
            var requiredRolesJson = await conn.ExecuteScalarAsync<string?>(
                new CommandDefinition(
            ...
            try
            {
                requiredRoles = requiredRolesJson is not null
                    ? JsonSerializer.Deserialize<string[]>(requiredRolesJson) ?? Array.Empty<string>()
                    : Array.Empty<string>();
            }
            catch { requiredRoles = Array.Empty<string>(); }

            var requiredRolesSet = new HashSet<string>(requiredRoles, StringComparer.OrdinalIgnoreCase);

            // Build WHERE clause for role filter
            var roleFilter = string.IsNullOrWhiteSpace(roleKey) ? "" : "AND ar.key = @roleKey";

            var admins = (await conn.QueryAsync<dynamic>(
                new CommandDefinition(
                    $"""
                    SELECT DISTINCT ON (aur.user_id)
                    ...
                    WHERE  1=1
                    {roleFilter}
                    ORDER  BY aur.user_id, ar.name
                    """,
                    new { roleKey },
                    cancellationToken: ct))).ToList();

            // Fetch MFA factors per user from Supabase (N+1 — acceptable for small admin sets)
            // Enrich ALL admins first, then filter by mfaStatus, then paginate so that
            // total reflects the post-filter count and pages are correctly sized.
            var items = new List<object>();
            foreach (var admin in admins)
            {
                Guid adminUserId = admin.user_id;
                var factors = new List<object>();
                bool mfaEnabled = false;

                try
                {
                    var factorsEl = await supabase.GetUserMfaFactorsAsync(adminUserId, ct);

                    // GoTrue may return array directly or wrapped: { "totp": [...] }
                    if (factorsEl.ValueKind == JsonValueKind.Array)
                    {
                        foreach (var f in factorsEl.EnumerateArray())
                        {
                            var fId     = f.TryGetProperty("id",            out var fid) ? fid.GetString() : null;
                            var fType   = f.TryGetProperty("factor_type",   out var ft)  ? ft.GetString()  : null;
                            var fStatus = f.TryGetProperty("status",        out var fs)  ? fs.GetString()  : null;
                            factors.Add(new
                            {
                                id           = fId,
                                type         = fType,
                                status       = fStatus,
                                createdAt    = f.TryGetProperty("created_at",    out var ca) ? ca.GetString() : null,
                                friendlyName = f.TryGetProperty("friendly_name", out var fn) ? fn.GetString() : null,
                            });
                            if (fStatus == "verified") mfaEnabled = true;
                        }
                    }
                    else if (factorsEl.ValueKind == JsonValueKind.Object)
                    {
                        foreach (var prop in factorsEl.EnumerateObject())
                        {
                ...
                {
                    // User has no MFA factors — expected; treat as no MFA
                }
                catch (Exception ex)
                {
                    // Log but continue — don't fail the whole request for one user's enrichment failure
                    var logger = ctx.RequestServices.GetRequiredService<ILoggerFactory>()
                        .CreateLogger("Esportra.Api.Endpoints.AdminEndpoints");
                    logger.LogWarning(ex, "GetUserMfaFactorsAsync failed for admin {UserId} — skipping enrichment", adminUserId);
                }

                // Apply mfaStatus filter after resolving real status
                if (mfaStatus == "enabled"  && !mfaEnabled) continue;
                if (mfaStatus == "disabled" &&  mfaEnabled) continue;

                string roleKeyVal = admin.role_key ?? "";
                items.Add(new
                {
                    userId      = adminUserId,
                    email       = (string?)admin.email,
                    fullName    = (string?)admin.full_name,
                    avatarUrl   = (string?)admin.avatar_url,
                    roleKey     = roleKeyVal,
                    roleName    = (string?)admin.role_name,
                    mfaEnabled,
                    mfaFactors  = factors,
                    requiresMfa = requiredRolesSet.Contains(roleKeyVal)
                });
            }

            // Post-filter total and pagination — total reflects only users matching the mfaStatus filter
            var total      = items.Count;
            var pagedItems = items.Skip((page - 1) * perPage).Take(perPage).ToList();

            return Results.Ok(new { users = pagedItems, total, page, perPage });
        }).RequireAuthorization("Admin");

        // ── POST /api/admin/2fa/settings ──────────────────────────────────────
        // Update 2FA enforcement settings.
        app.MapPost("/api/admin/2fa/settings", async (
            [FromBody] Save2faSettingsRequest req,
            IDbConnectionFactory             db,
            HttpContext                       ctx,
            CancellationToken                ct) =>
        {
            ...
            // Validate that all supplied role keys actually exist
            if (req.RequiredRoles.Length > 0)
            {
                var validKeys = (await conn.QueryAsync<string>(
                    new CommandDefinition(
                        "SELECT key FROM admin_roles WHERE key = ANY(@keys)",
                        new { keys = req.RequiredRoles },
                        cancellationToken: ct))).ToHashSet(StringComparer.OrdinalIgnoreCase);

                var invalid = req.RequiredRoles.Where(r => !validKeys.Contains(r)).ToArray();
                if (invalid.Length > 0)
                    return Results.BadRequest(new { error = $"Unknown role key(s): {string.Join(", ", invalid)}" });
            }

            var enabledValue = req.EnforcementEnabled ? "true" : "false";
            var rolesValue   = JsonSerializer.Serialize(req.RequiredRoles);

            await conn.ExecuteAsync(new CommandDefinition(
                """
                INSERT INTO system_settings (key, value, description, category, updated_at, updated_by)
                VALUES
                    ('security.2fa_enforcement_enabled', @enabledValue, 'Whether 2FA enforcement is active platform-wide',       'security', NOW(), @updatedBy),
                    ('security.2fa_required_roles',      @rolesValue,   'Roles that must have 2FA enabled (JSON array of role keys)', 'security', NOW(), @updatedBy)
                ON CONFLICT (key) DO UPDATE
                SET value      = EXCLUDED.value,
                    updated_at = NOW(),
                    updated_by = EXCLUDED.updated_by
                """,
                new { enabledValue, rolesValue, updatedBy = userCtx.UserIdGuid },
                cancellationToken: ct));

            // Audit log for 2FA settings change
            await conn.ExecuteAsync(new CommandDefinition(
                """
                INSERT INTO audit_logs (actor_id, action_type, target_type, target_id, details, created_at)
                VALUES (@actorId, '2fa_settings_updated', 'system', @actorId, @details::jsonb, NOW())
                """,
                new
                {
                    actorId = userCtx.UserIdGuid,
        ...
        // ── GET /api/admin/2fa/users/{userId}/factors ──────────────────────────
        // List all MFA factors for a specific user.
        app.MapGet("/api/admin/2fa/users/{userId:guid}/factors", async (
            Guid                 userId,
            ISupabaseAdminClient supabase,
            HttpContext          ctx,
            CancellationToken    ct) =>
        {
            ...
            JsonElement factorsEl;
            try
            {
                factorsEl = await supabase.GetUserMfaFactorsAsync(userId, ct);
            }
            ...
            if (factorsEl.ValueKind == JsonValueKind.Array)
            {
                foreach (var f in factorsEl.EnumerateArray())
                {
                    factors.Add(new
                    {
                        ...
                        createdAt    = f.TryGetProperty("created_at",    out var ca)  ? ca.GetString()  : null,
                        updatedAt    = f.TryGetProperty("updated_at",    out var ua)  ? ua.GetString()  : null,
                        type         = f.TryGetProperty("factor_type",   out var ft)  ? ft.GetString()  : null,
                        status       = f.TryGetProperty("status",        out var fs)  ? fs.GetString()  : null,
                        friendlyName = f.TryGetProperty("friendly_name", out var fn)  ? fn.GetString()  : null,
                    });
                }
            }
            ...
            return Results.Ok(new { userId, factors });
        }).RequireAuthorization("Admin");

        // ── DELETE /api/admin/2fa/users/{userId}/factors/{factorId} ───────────
        // Force-unenroll a specific MFA factor. Cannot remove own factors.
        app.MapDelete("/api/admin/2fa/users/{userId:guid}/factors/{factorId}", async (
            Guid                 userId,
            string               factorId,
            ISupabaseAdminClient supabase,
            IDbConnectionFactory db,
            ...
            CancellationToken    ct) =>
        {
            ...
            // Prevent admins from removing their own MFA factor via this endpoint
            if (userCtx.UserIdGuid == userId)
                return Results.BadRequest(new { error = "You cannot remove your own MFA factor via this endpoint." });

            if (string.IsNullOrWhiteSpace(factorId))
                return Results.BadRequest(new { error = "factorId is required." });

            if (!Guid.TryParse(factorId, out _))
                return Results.BadRequest(new { error = "factorId must be a valid UUID." });

            // Audit log written BEFORE the Supabase delete so it is always recorded
            // even if the delete call throws an exception.
            using var conn = db.CreateConnection();
            await conn.ExecuteAsync(new CommandDefinition(
                """
                INSERT INTO audit_logs (actor_id, action_type, target_type, target_id, details, created_at)
                VALUES (@actorId, 'admin_mfa_factor_removed', 'user', @targetId, @details::jsonb, NOW())
                """,
                new
                {
                    actorId  = userCtx.UserIdGuid,
                    ...
                    {
                        factorId,
                        removedBy = userCtx.Email,
                        action    = "admin_mfa_factor_removed"
                    })
                },
                cancellationToken: ct));
            ...
            try
            {
                await supabase.DeleteUserMfaFactorAsync(userId, factorId, ct);
            }
            catch (InvalidOperationException ex)
            {
                return Results.Problem(ex.Message, statusCode: 502);
...
// ── Phase 16: 2FA Enforcement ─────────────────────────────────────────────────
public sealed record Save2faSettingsRequest(bool EnforcementEnabled, string[] RequiredRoles);
