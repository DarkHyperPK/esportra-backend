...
-- NOTE: After 20260611120000_br_pro_lobby_model.sql, br_rounds is dropped and
-- br_round_evidence is renamed to br_lobby_evidence. Skip entire migration if br_rounds gone.

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'br_rounds' AND table_schema = 'public') THEN
        RAISE NOTICE 'br_rounds does not exist (dropped by br_pro_lobby_model), skipping staging repair';
        RETURN;
    END IF;

    -- Only runs if br_rounds still exists (pre-lobby-model state)
...
-- Policies and grants only if table exists
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'br_round_evidence' AND table_schema = 'public') THEN
        RETURN;
    END IF;
